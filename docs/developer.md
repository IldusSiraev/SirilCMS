...
**i18n интерфейса** — `payload.config.ts`, `i18n: { fallbackLanguage: 'en', supportedLanguages: { en, ru } }` (пакет `@payloadcms/translations`). Язык определяется по `Accept-Language` запроса; переключить можно в профиле пользователя (Settings → language). Кастомные/дополнительные строки — через `i18n.translations` (см. [Payload i18n docs](https://payloadcms.com/docs/configuration/i18n)). Лейблы полей блоков (`packages/blocks-definitions/src/registry.ts`, `BlockField.label`) и полей конструктора форм (`to-payload.ts`, `toPayloadFormFields`) — объект `{ ru, en }` (Payload `Field.label` как `Record<locale, string>`), переключаются вместе с языком UI. Контент сайта не переводится — `Sites.locale` (select `ru`/`en`) фиксирует язык ВСЕГО сайта целиком (мультиязычность внутри одного сайта не поддерживается, см. roadmap), используется на web для `<html lang>`.
