...
Одна запись на сайт. Поля: `name`, `slug`, `domain`, `locale`, **`theme`** (select из `THEMES` — см. §5.1), `contacts`, `logo` (media), `settings` (smtp/analytics — видны только owner). Изменение темы — **изменение данных, работает мгновенно, без деплоя**.
