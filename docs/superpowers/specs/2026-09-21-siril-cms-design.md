...
### Расширение набора блоков (новый блок — first-class)

Сценарий «клиент хочет блок, которого нет»:
1. `blocks-definitions`: тип — id, имя, schema полей, список вариантов (TS, единый источник).
2. Nuxt: компонент `blocks/<id>/` (варианты внутри компонента), стили через `tokens.css`.
3. Payload: `sections` — Blocks-поле, значения хранятся JSONB → **ноль миграций БД**; схема/типы админ-UI перегенерируются из `blocks-definitions`.
4. Тема: `blocks: {"<id>": {enabled: true}}` — блок виден в админке только сайтов с темой, где включён.
5. Один деплой Nuxt (правило: код = один раз на первое использование).

Оценка: 1–2 ч (простой блок), 0.5–1 день (сложный). Старые проекты не затрагиваются.
