...
export default defineEventHandler(async (event) => {
  ...
  const site = await resolveSite(getRequestHost(event) ?? '')
  if (!site) throw createError({ statusCode: 404, message: 'Form not found' })
  const { docs } = await payloadGet<{ docs: any[] }>(
    `forms?where[slug][equals]=${encodeURIComponent(slug!)}&where[site][equals]=${site.id}&depth=2`,
  )
  const form = docs[0]
  ...
  try {
    await $fetch(`${useRuntimeConfig().PAYLOAD_URL}/api/form-submissions`, {
      method: 'POST',
      body: { site: site.id, form: form.id, values, ip },
    })
  } catch {
